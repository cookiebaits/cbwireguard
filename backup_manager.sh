...
create_backup() {
    ...
    if tar -cz -C /etc wireguard | openssl enc -aes-256-cbc -salt -pbkdf2 -pass env:BACKUP_PASS -out "$BACKUP_FILE"; then
...
restore_backup() {
    ...
    if [[ "$index" =~ ^[0-9]+$ ]] && [ "$index" -ge 0 ] && [ "$index" -lt "${#BACKUP_FILES[@]}" ]; then
        ...
        if [[ "$confirm" =~ ^[Yy]$ ]]; then
            ...
            if openssl enc -aes-256-cbc -d -salt -pbkdf2 -pass env:BACKUP_PASS -in "$TARGET" | tar -xz -C "$TEMP_DIR"; then
